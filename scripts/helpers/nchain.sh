...
if [[ -z "${NAME}" ]]; then
    ...
    else echo " ${COLOR_RED}- NCHAIN is not supported for your Architecture!${COLOR_NC}" && exit 1
    fi
fi

# Setup yum and apt variables
if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
    if ! YUM=$( command -v yum 2>/dev/null ); then echo "${COLOR_RED}YUM must be installed to compile NCHAIN${COLOR_NC}" && exit 1; fi
elif [[ $NAME == "Ubuntu" ]]; then
    if ! APTGET=$( command -v apt-get 2>/dev/null ); then echo "${COLOR_RED}APT-GET must be installed to compile NCHAIN${COLOR_NC}" && exit 1; fi
fi

# Obtain dependency versions; Must come first in the script
. ${SCRIPT_DIR}/.environment
# Load general helpers
. ${SCRIPT_DIR}/helpers/general.sh
...
function setup() {
    if $VERBOSE; then
        ...
        echo "NCHAIN_INSTALL_DIR: ${NCHAIN_INSTALL_DIR}"
...
function ensure-which() {
  if ! which ls &>/dev/null; then
    while true; do
      [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}NCHAIN compiler checks require the 'which' package: Would you like for us to install it? (y/n)?${COLOR_NC}" && read -p " " PROCEED
      echo ""
      case $PROCEED in
...
function install-directory-prompt() {
    if [[ -z $INSTALL_LOCATION ]]; then
        echo "No installation location was specified. Please provide the location where NCHAIN is installed."
        while true; do
            [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to use the default location? ${NCHAIN_INSTALL_DIR}? (y/n)${COLOR_NC}" && read -p " " PROCEED
            echo ""
            case $PROCEED in
                "" )
                echo "What would you like to do?";;
                ...
                break;;
                1 | false | [Nn]* )
                printf "Enter the desired installation location." && read -p " " NCHAIN_INSTALL_DIR;
                export NCHAIN_INSTALL_DIR;
                break;;
                * ) echo "Please type 'y' for yes or 'n' for no.";;
            esac
        done
    else
        # Support relative paths : https://github.com/NCHAIN/eos/issues/7560
        [[ ! $INSTALL_LOCATION =~ ^\/ ]] && export INSTALL_LOCATION="${CURRENT_WORKING_DIR}/$INSTALL_LOCATION"
        export NCHAIN_INSTALL_DIR="$INSTALL_LOCATION"
    fi
    . ${SCRIPT_DIR}/.build_vars
    echo "NCHAIN will be installed to: ${NCHAIN_INSTALL_DIR}"
}

function previous-install-prompt() {
  if [[ -d $NCHAIN_INSTALL_DIR ]]; then
    echo "NCHAIN has already been installed into ${NCHAIN_INSTALL_DIR}... It's suggested that you nchain_uninstall.sh before re-running this script."
    while true; do
...
function resources() {
    echo "${COLOR_CYAN}NCHAIN website:${COLOR_NC} https://eos.io"
    echo "${COLOR_CYAN}NCHAIN Telegram channel:${COLOR_NC} https://t.me/EOSProject"
    echo "${COLOR_CYAN}NCHAIN resources:${COLOR_NC} https://eos.io/resources/"
    echo "${COLOR_CYAN}NCHAIN Stack Exchange:${COLOR_NC} https://nchain.stackexchange.com"
}

function print_supported_linux_distros_and_exit() {
   echo "On Linux the NCHAIN build script only supports Amazon, Centos, and Ubuntu."
...
function ensure-compiler() {
    ...
    if $PIN_COMPILER || [[ -f $CLANG_ROOT/bin/clang++ ]]; then
        ...
        if [[ $CXX =~ "clang" ]] || [[ $COMPILER_TYPE =~ "clang" ]]; then
            ...
            if [[ $NO_CPP17 == false ]]; then # https://github.com/NCHAIN/eos/issues/7402
                while true; do
                    echo "${COLOR_YELLOW}WARNING: Your GCC compiler ($CXX) is less performant than clang (https://github.com/NCHAIN/eos/issues/7402). We suggest running the build script with -P or install your own clang and try again.${COLOR_NC}"
                    [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to proceed anyway? (y/n)?${COLOR_NC}" && read -p " " PROCEED
                    case $PROCEED in
                        "" ) echo "What would you like to do?";;
                        0 | true | [Yy]* ) break;;
...
function ensure-cmake() {
    ...
    if [[ ! -e "${CMAKE}" ]]; then
        ...
        && ./bootstrap --prefix=${NCHAIN_INSTALL_DIR} \
        && make -j${JOBS} \
        && make install \
        && cd .. \
...
function ensure-boost() {
    ...
    if [[ $FOARCE_BOOST_COMPILE -ne 0 ]] || [[ "${BOOSTVERSION}" != "${BOOST_VERSION_MAJOR}0${BOOST_VERSION_MINOR}0${BOOST_VERSION_PATCH}" ]]; then
        ...
        if [[ -f "${BOOST_ROOT}/boost/version.hpp" ]]; then
            echo " -- Boost library -- source is exist @ ${BOOST_ROOT}"
        else
            if [[ ! -f "$SRC_DIR/boost_$BOOST_VERSION.tar.bz2" ]]; then
                BOOST_URL="https://dl.bintray.com/boostorg/release/$BOOST_VERSION_MAJOR.$BOOST_VERSION_MINOR.$BOOST_VERSION_PATCH/source/boost_$BOOST_VERSION.tar.bz2"
                echo " -- Boost library -- start to download boost source file from @ ${BOOST_URL}"
                execute bash -c "cd $SRC_DIR && curl -LO ${BOOST_URL}"
                [[ $? != 0 ]] && echo "Ensuring Boost download source file error." && exit 1
            fi
            echo " -- Boost library -- unpack source files @ ${BOOST_ROOT}"
            execute bash -c "tar -xjf boost_$BOOST_VERSION.tar.bz2"
            [[ $? != 0 ]] && echo "Ensuring Boost unpack source file error : $SRC_DIR/boost_$BOOST_VERSION.tar.bz2" && exit 1
        fi

        execute bash -c "cd $BOOST_ROOT \
        && SDKROOT="$SDKROOT" ./bootstrap.sh ${BOOTSTRAP_FLAGS} --prefix=$BOOST_ROOT \
        && SDKROOT="$SDKROOT" ./b2 ${B2_FLAGS}"

        [[ $? != 0 ]] && echo "Ensuring Boost building error." && exit 1
